    	<div class="title"><?=$text['sertificates']['title']?></div>
...
<?foreach ($text['sertificates']['content'] as $i => $sertificate) { ?>
        <a href="<?=$sertificate['image']?>" data-caption="<?=$sertificate['text']?>"><img src="<?=$sertificate['thumb']?>" height="64" width="96" alt="<?=$sertificate['text']?>"></a>
<?}?>
      </div>